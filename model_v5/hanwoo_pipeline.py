...
# ============================================================================
# 제출(v5)용 : test_hanwoo.csv 와 공통으로 존재하는 feature 만 사용
#   test 에는 도체 측정값(BACKFAT/REA/WINDEX/WGRADE/INSFAT/YUKSAK/FATSAK/TISSUE/
#   GROWTH/COST_AMT)이 없으므로 이를 절대 쓰지 않는다.
# ============================================================================
TEST_AVAILABLE_COLS = ["sido", "sigungu", "eupmyeondong", "stn", "ABATT_DATE", "JUDGE_DATE",
                       "JUDGE_SEX", "WEIGHT", "AGE", "BIRTH_YMD", "CATTLE_NO", "FARM_UNIQUE_NO"]


def _prep_base_min(df):
    df = normalize_missing(df)
    for c in ["WEIGHT", "AGE", "stn"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    return add_date_features(df)


def build_submission_frame(train_df, test_df, weather_df=None, area_df=None, death_df=None,
                           lineage_df=None, windows=(30, 90, 180), target="LAST_GRADE"):
    """train/test 양쪽에 공통 존재하는 feature 만으로 모델 입력 프레임을 만든다.
    반환: (parts={'train','test'}, numeric_features, categorical_features)
    모든 보간/빈도는 train 기준으로 fit → test 누수 차단."""
    train = _prep_base_min(train_df).dropna(subset=[target]).reset_index(drop=True)
    test = _prep_base_min(test_df).reset_index(drop=True)
    parts = {"train": train, "test": test}

    area_table = prepare_area_table(area_df)
    death_table = prepare_death_table(death_df)
    lineage_table = prepare_lineage_table(lineage_df)
    for k in parts:
        parts[k] = merge_area_features(parts[k], area_table)
        parts[k] = merge_death_features(parts[k], death_table)
        parts[k] = merge_lineage_features(parts[k], lineage_table)

    weather_features = None
    if weather_df is not None:
        wd = prepare_weather_daily(weather_df)
        weather_features = build_weather_rolling_features(wd, windows)
        weather_features["stn"] = pd.to_numeric(weather_features["stn"], errors="coerce").astype("Int64")
    for k in parts:
        parts[k] = merge_weather_features(parts[k], weather_features)
    ...
    # cow_year/AREA : 지역 median fallback
    for col in ["cow_year", "AREA"]:
        spec = fit_fallback_imputer(parts["train"], col, [["sigungu"], ["sido"]], kind="median", numeric=True)
        for k in parts:
            parts[k] = apply_fallback_imputer(parts[k], spec)
    for k in parts:
    ...
    numeric_features = (["WEIGHT", "AGE", "rearing_days", "judge_delay_days", "cow_year",
                         "log_cow_year", "log_area", "farm_death_count",
                         "kpn_known", "kpn_freq", "log_kpn_freq"] + weather_cols)
    numeric_features = [c for c in numeric_features if c in parts["train"].columns]
    categorical_features = [c for c in ["JUDGE_SEX", "sido", "sigungu", "stn", "abatt_month", "abatt_season"]
                            if c in parts["train"].columns]
    return parts, numeric_features, categorical_features
